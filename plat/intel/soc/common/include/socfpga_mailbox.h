...
#define MBOX_CMD_BUFFER					0x40 /* Circular buffer containing commands to SDM */
#define MBOX_RESP_BUFFER				0xC0 /* Circular buffer containing responses from SDM */
...
#define IS_CMD_DIRECT(cmd)				((((cmd) & MBOX_CMD_FLAG_DIRECT) != 0) ? 1 : 0)
#define IS_CMD_INDIRECT(cmd)				((((cmd) & MBOX_CMD_FLAG_INDIRECT) != 0) ? 1 : 0)
#define IS_CMD_CASUAL(cmd)				((((cmd) & MBOX_CMD_FLAG_CASUAL) != 0) ? 1 : 0)
#define IS_CMD_URGENT(cmd)				((((cmd) & MBOX_CMD_FLAG_URGENT) != 0) ? 1 : 0)
...
typedef struct sdm_cmd_ret_arg {
	uint8_t ret_args_len;
	uint32_t ret_args[MBOX_SVC_SMC_RET_MAX_SIZE];
	sdm_response_t* response;
} sdm_cmd_ret_arg_t;
...
typedef struct sdm_command {
	...
	uint32_t *cb_args;
	uint8_t cb_args_len;
	sdm_cmd_ret_arg_t cmd_ret_args;
} sdm_command_t;
...
typedef struct mailbox_service {
	...
	uint32_t flags;
	int curr_di;
	uint64_t received_bitmap[MBOX_MAX_TRANS_IDS_BITMAP];
	uint64_t interrupt_bitmap[MBOX_MAX_TRANS_IDS_BITMAP];
